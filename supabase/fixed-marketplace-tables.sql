...
-- Add similar_to_item_id column if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        ...
        AND column_name = 'similar_to_item_id'
    ) THEN
        ALTER TABLE public.products ADD COLUMN similar_to_item_id UUID;
    END IF;
END $$;
...
-- Drop policies if they already exist (to avoid duplicates)
DROP POLICY IF EXISTS "Users can view their own wishlist items" ON public.wish_list;
DROP POLICY IF EXISTS "Users can insert their own wishlist items" ON public.wish_list;
DROP POLICY IF EXISTS "Users can update their own wishlist items" ON public.wish_list;
DROP POLICY IF EXISTS "Users can delete their own wishlist items" ON public.wish_list;
...
-- Drop policies if they already exist (to avoid duplicates)
DROP POLICY IF EXISTS "Users can insert their own click tracking" ON public.click_tracking;
...
-- Drop policies if they already exist (to avoid duplicates)
DROP POLICY IF EXISTS "Users can view their own product recommendations" ON public.product_recommendations;
DROP POLICY IF EXISTS "Users can update their own product recommendations" ON public.product_recommendations;
...
-- Drop policies if they already exist (to avoid duplicates)
DROP POLICY IF EXISTS "Price history is viewable by all authenticated users" ON public.price_history;
...
-- Create indexes for performance (drop first to avoid errors if they already exist)
DROP INDEX IF EXISTS products_category_idx;
DROP INDEX IF EXISTS products_similar_to_item_id_idx;
DROP INDEX IF EXISTS wish_list_user_id_idx;
DROP INDEX IF EXISTS click_tracking_product_id_idx;
DROP INDEX IF EXISTS product_recommendations_user_id_idx;
DROP INDEX IF EXISTS price_history_product_id_idx;
