...
import { Camera, Upload, X, RefreshCw, Tag, Check, Trash } from 'lucide-react';
...
export default function UploadPage() {
  ...
  const [analysisMode, setAnalysisMode] = useState<string>('tag');
  ...
  const startCamera = async () => {
    try {
      ...
      setCameraActive(true); // Just activate the camera UI, will handle video in useEffect
    } catch (err) {
  ...
  // Add a useEffect hook to handle the video element after it's rendered
  useEffect(() => {
  ...
  const analyzeImage = async (imageData: string) => {
    ...
    try {
      // First try our new testpages API endpoint
      const res = await fetch('/api/testpages/clothes-finder', {
        method: 'POST',
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to analyze the image';
      setError(`${errorMessage}. You can add tags manually.`);
  ...
  const addManualTag = () => {
    if (manualTagInput.trim()) {
      setTags([...tags, manualTagInput.trim()]);
      setManualTagInput('');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Upload & Camera</h1>
      
      {/* Mode selector */}
      <div className="mb-4">
        <h2 className="text-sm font-medium mb-2">Analysis Type:</h2>
        <div className="flex space-x-2">
          {['tag', 'detail', 'style'].map((mode) => (
            <button
              key={mode}
        ...
        <div className="bg-gray-100 dark:bg-gray-800 rounded-lg p-4 mb-4">
          {cameraActive ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-lg font-medium">Capture or upload a fashion image</p>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                We&apos;ll analyze your clothing items and provide fashion insights
              </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="mt-2 flex justify-between">
                <button
                  onClick={resetAll}
                  className="flex items-center px-3 py-1 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 text-sm"
                >
            ...
            <div>
              ...
              {loading ? (
                <div className="flex items-center justify-center h-40 bg-white dark:bg-gray-900 rounded-md p-4">
                  <div className="animate-spin rounded-full h-8 w-8 border-2 border-t-blue-500 border-gray-300"></div>
                </div>
              ) : error ? (
                <div className="bg-red-100 dark:bg-red-900/20 p-3 rounded-md text-red-700 dark:text-red-400 text-sm mb-3">
                  {error}
                </div>
              ) : null}
                    ...
                    {tags.map((tag, index) => (
                      ...
                      >
                        ...
                        <button 
                          ...
                          className="ml-1 text-blue-700 dark:text-blue-400 hover:text-red-500"
                        >
                          <X size={14} />
                        </button>
                      </div>
              ...
              {/* Manual tag input */}
              {(showManualTagInput || tags.length > 0) && (
                <div className="mt-3">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="px-3 py-1 border border-gray-300 dark:border-gray-700 rounded-md flex-1 bg-white dark:bg-gray-800"
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg text-sm">
        <h3 className="font-medium mb-1">About this feature:</h3>
        <p className="text-gray-700 dark:text-gray-300">
          This component implements a comprehensive upload and camera integration with AI-powered clothing analysis.
          It uses the Gemini API to analyze clothing items and accessories in your images.
        </p>
        <p className="text-gray-700 dark:text-gray-300 mt-2">
          The analysis modes offer different levels of detail:
        </p>
        <ul className="list-disc pl-5 mt-1 space-y-1 text-gray-700 dark:text-gray-300">
          <li><strong>Tag</strong>: Simple list of clothing items and colors</li>
          <li><strong>Detail</strong>: Comprehensive description of each item</li>
          <li><strong>Style</strong>: Overall style analysis and recommendations</li>
        </ul>
      </div>
    </div>
  );
