...
export default function BattlePage() {
  ...
  const handleImageError = (lookId: string) => {
    setImageErrors(prev => ({ ...prev, [lookId]: true }));
  };
                    ...
                    {battle.map((look, index) => (
                      <div key={look.look_id} className="relative">
                        <div className="relative aspect-square rounded-md overflow-hidden">
                          {!imageErrors[look.look_id] ? (
                            <SupabaseImage
                              src={look.image_url}
                              alt={look.caption || "Fashion look"}
                              fill
                              className="object-cover"
                              onError={() => handleImageError(look.look_id)}
                            />
                          ) : (
                        ...
                        <div className="mt-2">
                          <p className="text-sm font-medium truncate">{look.caption || "Fashion Look"}</p>
                          <p className="text-xs text-gray-500 truncate">{look.username || "Anonymous User"}</p>
                        </div>
