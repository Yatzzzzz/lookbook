  ...
  useEffect(() => {
    async function fetchBattleItems() {
      try {
        ...
        // Check the battle storage bucket
        ...
        // First check if there are any files in the root directory
        ...
        if (imageFiles.length > 0) {
          ...
          // Group the files by prefix to create battle sets
          ...
          for (const file of imageFiles) {
            ...
            if (file.name.toLowerCase().includes('main')) {
              fileType = 'main';
            } else if (file.name.toLowerCase().includes('option1')) {
              fileType = 'option1';
            } else if (file.name.toLowerCase().includes('option2')) {
              fileType = 'option2';
            }
            
            // Ensure we have a valid ID for grouping
            if (!filePrefix) {
              filePrefix = 'default';
            }
            
            // Create or update battle set
            if (!battleSets[filePrefix]) {
              battleSets[filePrefix] = {
                id: filePrefix,
                username: 'User',
            ...
            // Add the URL to the appropriate field
            if (fileType === 'main') {
              battleSets[filePrefix].image_url = urlData.publicUrl;
            } else if (fileType === 'option1') {
              battleSets[filePrefix].option1_url = urlData.publicUrl;
            } else if (fileType === 'option2') {
              battleSets[filePrefix].option2_url = urlData.publicUrl;
            }
          }
          
          // Convert battle sets to array
          const items = Object.values(battleSets).filter(item => 
            // Only include items that have at least one option
            item.option1_url || item.option2_url
          );
          
          if (items.length > 0) {
            console.log(`Created ${items.length} battle items from root images`);
        ...
        // If no files in root, check for user folders
        const folders = storageData?.filter(item => !item.name.includes('.')) || [];
        console.log(`Found ${folders.length} potential user folders`);
        
        if (folders.length === 0) {
          setError('No battle images found. Upload battle images to your Supabase storage.');
          setIsLoading(false);
          return;
        }
        
        // Process each folder
        const allItems: BattleItem[] = [];
        
        for (const folder of folders) {
          const username = folder.name;
          console.log(`Processing folder: ${username}`);
          ...
          if (imageFiles.length === 0) {
            console.log(`No image files found in folder ${username}`);
            continue;
          }
          
          // Try to group related files by timestamp or prefix
          const battleSets: Record<string, BattleItem> = {};
          ...
          for (const file of imageFiles) {
            ...
            console.log(`Generated public URL for ${username}/${file.name}: ${urlData.publicUrl}`);
            
            // Try to determine set ID and file type
            let setId = file.name.split('.')[0]; // Default to filename without extension
            let fileType = 'main';
            
            // Check for pattern in filename
            if (file.name.includes('-')) {
              const parts = file.name.split('-');
              if (parts.length >= 2) {
                if (/^\d+$/.test(parts[0])) {
                  // First part is numeric, likely a timestamp
                  setId = parts[0];
                  // Check second part for type
                  ...
                  // First part might be the type
                  const typePart = parts[0].toLowerCase();
                  if (typePart.includes('main')) {
                    fileType = 'main';
                    setId = parts[1].split('.')[0];
                  } else if (typePart.includes('option1')) {
                    fileType = 'option1';
                    setId = parts[1].split('.')[0];
                  } else if (typePart.includes('option2')) {
              ...
              // Simple filename, check for type indicators
              const fileName = file.name.toLowerCase();
              if (fileName.includes('main')) {
                fileType = 'main';
              } else if (fileName.includes('option1')) {
                fileType = 'option1';
              } else if (fileName.includes('option2')) {
                fileType = 'option2';
              }
            }
            
            // Create or update battle set entry
            const fullSetId = `${username}-${setId}`;
            if (!battleSets[fullSetId]) {
              battleSets[fullSetId] = {
                id: fullSetId,
                username: username,
                caption: 'Which option do you prefer?'
              };
            ...
            // Add the URL to the appropriate field
            if (fileType === 'main') {
              battleSets[fullSetId].image_url = urlData.publicUrl;
            } else if (fileType === 'option1') {
              battleSets[fullSetId].option1_url = urlData.publicUrl;
            } else if (fileType === 'option2') {
              battleSets[fullSetId].option2_url = urlData.publicUrl;
            }
          }
          
          // Add valid battle sets to items
          Object.values(battleSets).forEach(set => {
            // A valid set needs at least one option
            if (set.option1_url || set.option2_url) {
              // If no main image, use first option as main
              if (!set.image_url) {
                set.image_url = set.option1_url || set.option2_url;
              }
              allItems.push(set);
            }
          });
        ...
        if (allItems.length > 0) {
          console.log(`Created ${allItems.length} battle items from user folders`);
          setBattleItems(allItems);
        } else {
          setError('No valid battle images found. Each battle needs at least one option image.');
        }
      } catch (error) {
