...
export async function GET(_: NextRequest) {
  try {
    ...
    // First check if RLS is causing issues on look_ratings table
    // by trying to fix the policies
    await supabaseAdmin.rpc('query', { 
      query: `
        ...
        -- Make sure looks table has the feature_in column
        DO $$
        BEGIN
          ...
          -- Ensure any null feature_in values are converted to arrays
          UPDATE looks SET feature_in = ARRAY['gallery'] WHERE feature_in IS NULL;
        END $$;
        
        -- Make sure the rating_count and avg_rating columns exist in looks table
        DO $$
        BEGIN
          IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        -- Fix any update_look_rating_stats() issues
