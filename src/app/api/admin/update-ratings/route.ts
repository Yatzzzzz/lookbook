...
// Function to recreate the database triggers and functions if needed
const recreateTriggers = async (supabase: any) => {
  try {
    // This SQL recreates the key triggers and functions
    const sql = `
      ...
      BEGIN
          ...
          -- Update the looks table with new statistics using the rating column
          UPDATE looks
          SET 
      ...
      -- Function to update the top_rated_look flag
      ...
      BEGIN
          ...
          -- Add top_rated flag to the top 50 rated looks (using a subquery to count ratings)
...
// Function to synchronize all ratings
const syncAllRatings = async (supabase: any) => {
  try {
    // This synchronizes all look ratings to only use the rating column
    const sql = `
      ...
      UPDATE looks l
      SET 
        rating = trs.avg_rating::TEXT
      FROM temp_rating_stats trs
      WHERE l.look_id = trs.look_id;

      -- Run the top rated update
      SELECT update_top_rated_flags();

      -- Get stats about how many records were updated
      SELECT 
        COUNT(*) AS total_looks,
        (SELECT COUNT(DISTINCT look_id) FROM look_ratings) AS rated_looks,
        (SELECT ROUND(AVG(cnt)) FROM (SELECT COUNT(*) AS cnt FROM look_ratings GROUP BY look_id) AS x) AS avg_ratings_per_look,
        (SELECT MAX(cnt) FROM (SELECT COUNT(*) AS cnt FROM look_ratings GROUP BY look_id) AS x) AS max_ratings_on_look
      FROM looks;
    `;
  ...
  try {
    ...
    return NextResponse.json({
      ...
      stats: data || {
        total_looks: 0,
        rated_looks: 0,
        avg_ratings_per_look: 0,
        max_ratings_on_look: 0
      }
    });
  } catch (err: any) {
    console.error('Error in rating stats API:', err);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'recreate_triggers') {
      const result = await recreateTriggers(supabase);
      if (!result.success) {
        return NextResponse.json(
          { error: 'Failed to recreate triggers', details: result.error },
          { status: 500 }
        );
      }
      return NextResponse.json({
        success: true,
        message: 'Database triggers recreated successfully'
      });
    } else if (action === 'sync_ratings') {
      const result = await syncAllRatings(supabase);
      if (!result.success) {
        return NextResponse.json(
          { error: 'Failed to sync ratings', details: result.error },
          { status: 500 }
        );
      }
      return NextResponse.json({
      ...
      return NextResponse.json(
        { error: 'Invalid action. Must be "recreate_triggers" or "sync_ratings"' },
        { status: 400 }
      );
    }
  } catch (err: any) {
    console.error('Error in update ratings API:', err);
    return NextResponse.json(
      { error: 'Internal server error', details: err.message },
      { status: 500 }
    );
