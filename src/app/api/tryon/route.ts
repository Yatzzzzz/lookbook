...
export async function POST(request: NextRequest) { 
  try { 
    ...
    let { imageBase64, outfitUrl, outfitBase64, type = 'full', modelVersion = 'v1.5' } = requestBody;
    ...
    if (outfitBase64) {
      ...
      return NextResponse.json(
        { 
          ...
          details: 'Either cloth_image or cloth_image_url must be provided'
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      try {
        ...
        return NextResponse.json(
          { 
            error: 'Invalid response format from KlingAI API',
      ...
      if (!response.ok) {
        ...
        return NextResponse.json(
          { 
            error: data.message || 'Error from KlingAI API',
            details: data
          },
      ...
      while (retryCount < maxRetries) {
        ...
        if (statusData.data?.status === 'FAILED') {
          ...
          return NextResponse.json(
            { 
              error: 'KlingAI task failed',
              details: statusData.data.result
            },
            { status: 500 }
          );
        }
      ...
      if (!resultImageUrl) {
        ...
        return NextResponse.json(
          { 
            ...
            details: 'The image processing task did not complete in the expected time.'
          },
          { status: 504 }
        );
      }
      ...
      return NextResponse.json({ success: true, resultImageUrl: resultImageUrl });
      
    } catch (error) {
      console.error('Error processing KlingAI request:', error);
      return NextResponse.json(
        { 
          error: 'Error processing try-on request',
