...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: wardrobeItems, error: wardrobeError } = await supabase
      .from('wardrobe')
      .select('item_id, user_id, image_path, name, description, category, brand, style')
      .or(`name.ilike.%${searchQuery}%,description.ilike.%${searchQuery}%,category.ilike.%${searchQuery}%,brand.ilike.%${searchQuery}%,style.ilike.%${searchQuery}%`)
      .limit(limit);
    ...
    if (!wardrobeError && wardrobeItems && wardrobeItems.length > 0) {
      ...
      wardrobeItems.forEach(item => {
        results.push({
          id: item.item_id,
          type: 'wardrobe',
          primary_text: item.name || item.category || 'Wardrobe Item',
          secondary_text: item.brand ? `${item.brand} - ${item.category || ''}` : (item.category || ''),
